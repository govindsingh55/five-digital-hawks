...
<article class="pt-32 pb-20 min-h-screen">
  <div class="container mx-auto px-4 max-w-7xl">
    ...
    <a
      href="/blog"
      class="btn btn-ghost btn-sm rounded-full gap-2 mb-8 hover:bg-base-200"
    >
      <ArrowLeft size={16} />
      Back to Blog
    </a>
    ...
    <header class="mb-12">
      ...
      <h1
        class="text-4xl md:text-5xl font-bold text-brand-primary leading-tight mb-8"
      >
        {data.meta.title}
      </h1>
    ...
    <!-- Content -->
    <div
      class="prose prose-lg md:prose-xl max-w-none prose-headings:text-brand-primary prose-a:text-brand-accent hover:prose-a:text-brand-highlight prose-img:rounded-3xl prose-pre:bg-slate-900 prose-pre:shadow-lg"
    >
      <data.content />
    </div>
    ...
    <footer class="mt-20 pt-10 border-t border-base-300">
      <div class="flex flex-wrap gap-2 mb-8">
        {#each data.meta.categories as category}
          <span class="badge badge-primary badge-outline px-4 py-3"
            >{category}</span
          >
        {/each}
      </div>
...
<style>
  ...
  @media (min-width: 768px) {
    :global(.prose h1) {
      font-size: 3rem; /* match md:text-5xl */
    }
  }
  :global(.prose h2) {
    margin-top: 2.5em;
